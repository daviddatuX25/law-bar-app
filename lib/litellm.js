...
const DEFAULTS = {
  ...
  timeout: 60000,  // 60s (DeepSeek v4 reasoning model needs more time)
  retries: 2,
};
...
async function callLiteLLM(systemPrompt, userPrompt, options = {}) {
  ...
  try {
    ...
    // DeepSeek v4 with merge_reasoning_content_in_choices=false sends
    // reasoning_content separately. Log it for debugging but don't use it.
    if (reasoningContent) {
      console.log(`[litellm] Reasoning tokens: ${reasoningContent.length} chars`);
    }

    // If content is empty but reasoning_content exists, the model ran out
    // of tokens during reasoning. This means max_tokens is too low.
    if (!content && reasoningContent) {
      console.warn(`[litellm] Empty content — model exhausted tokens during reasoning. Increase max_tokens (current: ${maxTokens}, need ~8000 for DeepSeek v4)`);
    }

    const usage = data.usage || {};
    const actualModel = data.model || model;

    return { content, usage, model: actualModel, reasoningContent };
  } catch (err) {
    clearTimeout(timer);
...
FRONT (shape): Abstracted fact pattern related to prompt: "${sanitizedPrompt}" (Mock #${i + 1}).
...
SOURCE_PARAGRAPH: ${pId}`);
  ...
  try {
    const response = await callLiteLLM(systemPrompt, userPrompt, {
      ...
      maxTokens: Math.max(DEFAULTS.maxTokens, 4000),
      temperature: 0.2,
      responseFormat: null
    });
